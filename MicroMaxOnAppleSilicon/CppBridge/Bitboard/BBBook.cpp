...
static const int PieceIdx     = 0;
static const int CastleIdx    = 768;
static const int TurnIdx      = 780;
static const int EnPassantIdx = 772;
...
BBBook::BBBook()
{
    for (auto i = abs(get_system_time() % 10000); i > 0; i--)
    {
        RKiss.rand<unsigned>();
    }
}
...
void BBBook::open(const string& fileName)
{
    ...
    bookSize = int(bookFile.tellg()) / sizeof(BookEntry);

    if (!bookFile.good())
    {
        throw "Failed to open book file";
        //cerr << "Failed to open book file " << fileName << endl;
        //exit(EXIT_FAILURE);
    }
}

// Reference: http://stackoverflow.com/questions/279854/how-do-i-sort-a-vector-of-pairs-based-on-the-second-element-of-the-pair
struct sortPred
{
...
void BBBook::query(const Position& pos, std::vector<std::pair<Move, float> > &moves)
{
    ...
    // Query for every single candidate book move
    for (int idx = find_entry(key); idx < bookSize; idx++)
    {
        ...
        const int p = (tempBookMove >> 12) & 7;

        if ((tempBookMove >> 12) & 7)
        {
            tempBookMove = int(make_promotion_move(move_from(Move(tempBookMove)), move_to(Move(tempBookMove)), PieceType(p + 1)));
        }

        totalScore += entry.count;
        pairs.push_back(std::make_pair(tempBookMove, entry.count));
    }

    // Sort the pairs in increasing orders based on the scores
    std::sort(pairs.begin(), pairs.end(), sortPred());

    MoveStack mlist[MAX_MOVES];
    MoveStack* last = generate<MV_LEGAL>(pos, mlist);

    for (std::vector<std::pair<int, float> >::const_iterator iter = pairs.begin(); iter != pairs.end(); iter++)
    {
        int bookMove = iter->first;

        for (MoveStack* cur = mlist; cur != last; cur++)
        {
            if ((int(cur->move) & ~(3 << 14)) == bookMove) // Mask out special flags
            {
                moves.push_back(std::make_pair(cur->move, iter->second / totalScore));
            }
        }
    }
}
...
Move BBBook::get_move(const Position& pos, bool findBestMove)
{
    if (!bookFile.is_open() || bookSize == 0)
    {
        return MOVE_NONE;
    ...
    if ((bookMove >> 12) & 7)
    {
        bookMove = int(make_promotion_move(move_from(Move(bookMove)), move_to(Move(bookMove)), PieceType(p + 1)));
    }
...
int BBBook::find_entry(uint64_t key)
{
    int left, right, mid;
...
BookEntry BBBook::read_entry(int idx)
{
    ...
    get_number(entry.key);
    get_number(entry.move);
    get_number(entry.count);
    get_number(entry.learn);

    if (!bookFile.good())
    {
        throw std::runtime_error("Invalid book file");
    }
