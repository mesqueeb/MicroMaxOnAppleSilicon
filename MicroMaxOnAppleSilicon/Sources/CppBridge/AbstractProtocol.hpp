...
namespace Framework
{
	class AbstractProtocol : public Protocol
	{
		public:
			...
			static void convertCoordToStr(char buffer[], size_t bufferSize, unsigned int srcFile, unsigned int srcRank, unsigned int dstFile, unsigned int dstRank, unsigned promote)
			{
        snprintf(buffer, bufferSize, "%c%d%c%d", CONVERT_FILE_TO_CHAR(srcFile), srcRank + 1, CONVERT_FILE_TO_CHAR(dstFile), dstRank + 1);
